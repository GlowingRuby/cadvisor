...
type Plugin interface {
	...
	Register(factory info.MachineInfoFactory, fsInfo fs.FsInfo, includedMetrics MetricSet, idWhiteList []string) (watcher.ContainerWatcher, error)
}
...
func InitializePlugins(factory info.MachineInfoFactory, fsInfo fs.FsInfo, includedMetrics MetricSet, idWhiteList []string) []watcher.ContainerWatcher {
	pluginsLock.Lock()
	defer pluginsLock.Unlock()

	containerWatchers := []watcher.ContainerWatcher{}
	for name, plugin := range plugins {
		watcher, err := plugin.Register(factory, fsInfo, includedMetrics, idWhiteList)
		if err != nil {
